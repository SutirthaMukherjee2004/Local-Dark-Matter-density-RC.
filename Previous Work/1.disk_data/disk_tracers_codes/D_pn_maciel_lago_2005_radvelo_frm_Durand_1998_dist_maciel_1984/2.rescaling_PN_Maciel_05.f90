...
open(unit=1,file='l_b_HRV_EHRV_D_data6_10pnt_excld_Maciel05_ref_Durand98_Maciel84',status='unknown')
open(unit=2,file='8.0_200_l_b_r_vc_x_y_z_maciel',status='unknown')
open(unit=3,file='8.0_200_rmid_vcmean_errvc_maciel',status='unknown')
...
xx=x5*dcos(x2*pi/180.0)*dsin(x1*pi/180.0)
yy=rsun-x5*dcos(x2*pi/180.0)*dcos(x1*pi/180.0)
zz=x5*dsin(x2*pi/180.0)
rreq=dsqrt(xx**2.d0+yy**2.d0)

if( (x1.gt.15.0.and.x1.lt.345.0) .and. (dabs(x2).lt.10.0).and.(dabs(zz).le.2.d0).and.(rreq.le.25.d0)) then
