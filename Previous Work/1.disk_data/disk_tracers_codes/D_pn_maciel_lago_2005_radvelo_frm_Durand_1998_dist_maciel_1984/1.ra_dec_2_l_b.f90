...
open(unit=1,file='data5bex.HRV_EHRV_RA_DEC_D_PN_487_Maciel05_ref_Durand98_Maciel84',status='unknown')
open(unit=2,file='data6bex.l_b_HRV_EHRV_D_Maciel05_ref_Durand98_Maciel84',status='unknown')

pi=3.141592653589793d0
nd=0


 19	read(1,*,iostat=ieof1)x1,x2,x3,x4,x5
       	print*,x1,x2,x3,x4,x5,nd
	if(ieof1.eq.0)then
	nd=nd+1
	vh(nd)=x1
	evh(nd)=x2
...
al(j)=angle
write(2,*)al(j)*180.0/pi,bcal(j)*180.0/pi,vh(j),evh(j),rh(j)
enddo
