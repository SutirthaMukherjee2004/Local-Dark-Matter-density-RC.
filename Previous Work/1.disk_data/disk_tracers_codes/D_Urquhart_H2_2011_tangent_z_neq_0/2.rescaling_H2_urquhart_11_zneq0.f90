	...
	open(1,file='l_b_vt_Urquhart_2011_HII',status='unknown')


	open(unit=2,file='8.5_220_l_b_r_vc_x_y_z_Urquhart_HII',status='unknown')
	open(unit=3,file='8.0_200_l_b_r_vc_x_y_z_Urquhart_HII',status='unknown')
	open(unit=4,file='8.3_244_l_b_r_vc_x_y_z_Urquhart_HII',status='unknown')


        open(40,file='8.5_220_rmid_vcmean_errvc_Urquhart_HII',status='unknown')
        open(50,file='8.0_200_rmid_vcmean_errvc_Urquhart_HII',status='unknown')
        open(60,file='8.3_244_rmid_vcmean_errvc_Urquhart_HII',status='unknown')
...
!
	...
	vsun3=244.d0
        ...
        do i=1,imax
	!if((x1.ge.10.0.and.x1.le.170.0) .or. (x1.ge.190.0.and.x1.le.350.0)) then
        read(1,*)al(i),b(i),vtan(i)
        end do

       	do i=1,imax

! rreq cut not req as all points lie within solar circle and z cut not req as z's are very small.
	al(i)=al(i)*pi/180.d0
	b(i)=b(i)*pi/180.d0
	...
	!if (al(i).ge.(1.5d0*pi).and.al(i).lt.2.d0*pi) then
	
	vc1(i)=dabs((vtan(i)/dcos(b(i)))+vsun1*sin(al(i)))
	vc2(i)=dabs((vtan(i)/dcos(b(i)))+vsun2*sin(al(i)))
	vc3(i)=dabs((vtan(i)/dcos(b(i)))+vsun3*sin(al(i)))
	...
	write(2,*)al(i)*180.0/pi,b(i)*180.0/pi,dis1(i),vc1(i),r1*cos(al(i)),r1*sin(al(i)),rsun1*cos(al(i))*tan(b(i))
	write(3,*)al(i)*180.0/pi,b(i)*180.0/pi,dis2(i),vc2(i),r2*cos(al(i)),r2*sin(al(i)),rsun2*cos(al(i))*tan(b(i))
	write(4,*)al(i)*180.0/pi,b(i)*180.0/pi,dis3(i),vc3(i),r3*cos(al(i)),r3*sin(al(i)),rsun3*cos(al(i))*tan(b(i))
	end do
